  ...
  (str (h/html [:html
                [:div {:style "margin:0 auto;width: 400px;text-align: center;"}
                 ...
                 [:p (if (or (= 2 mode) (and (= 4 mode) (some? first-ai-level)))
                       (second prompts/level-prompt)
                       (first prompts/level-prompt))]
                 [:form {:method "POST" :action "/ttt"}
                  [:select {:name (if (and (= mode 4) (some? first-ai-level))
                                    "second-ai-level"
                                    "first-ai-level")}
                   [:option {:value 1} (nth prompts/level-prompt 3)]
                   [:option {:value 2} (nth prompts/level-prompt 4)]
                   [:option {:value 3} (nth prompts/level-prompt 6)]]
                  [:input {:type "submit"}]]]])))
